...
from magi_core.utils import EmbeddingFunc, Tokenizer, TokenizerInterface
...
def _patch_prompt_dir(path: Path):
    return patch("magi_core.prompt.get_entity_type_prompt_dir", return_value=path)
...
def test_default_entity_types_removed_from_constants():
    """DEFAULT_ENTITY_TYPES must no longer exist in magi_core.constants."""
    import magi_core.constants as constants
...
async def test_text_mode_default_guidance_injected_into_prompt():
    ...
    from magi_core.operate import extract_entities
    from magi_core.prompt import PROMPTS
    ...
    with patch("magi_core.operate.logger"):
...
async def test_rebuild_from_cached_fenced_json_uses_json_parser():
    ...
    from magi_core import operate

    fenced_json = f"```json\n{_JSON_MODE_RESPONSE}\n```"

    with patch(
        "magi_core.operate._process_extraction_result",
...
async def test_extract_entities_uses_cached_prompt_profile_without_reloading():
    ...
    with patch(
        "magi_core.operate.resolve_entity_extraction_prompt_profile",
...
def test_prompt_dir_defaults_to_cwd_relative(tmp_path, monkeypatch):
    ...
    from magi_core.prompt import get_entity_type_prompt_dir
...
def test_prompt_file_malformed_yaml_raises_valueerror(tmp_path):
    ...
    from magi_core.prompt import load_entity_extraction_prompt_profile
...
def test_addon_guidance_overrides_file_profile(tmp_path):
    ...
    from magi_core.prompt import resolve_entity_extraction_prompt_profile
...
def test_format_heading_context_full_path_includes_current_heading():
    ...
    from magi_core.chunk_schema import format_heading_context
...
def test_format_parent_headings_per_level_cap_can_be_disabled():
    from magi_core.chunk_schema import format_parent_headings
...
async def test_attach_content_headings_token_budgets_deep_path():
    ...
    from magi_core.chunk_schema import HEADING_BREADCRUMB_SEP
    from magi_core.constants import DEFAULT_MAX_SECTION_CONTEXT_TOKENS
    from magi_core.operate import _attach_content_headings
...
def test_truncate_section_context_noop_within_budget():
    from magi_core.operate import _truncate_section_context
...
async def test_extract_entities_bounds_pathological_heading_in_prompt():
    ...
    from magi_core.chunk_schema import DEFAULT_HEADING_LEVEL_MAX_CHARS
    from magi_core.operate import extract_entities
...
def test_clean_heading_text_converts_arrow_to_space():
    ...
    from magi_core.chunk_schema import _clean_heading_text
