...
# import time of magi_core.api.config). Tests must be hermetic and not depend
...
def _isolate_env(monkeypatch):
    ...
    The magi_core.api.config module loads .env at import time, which can leave
...
def mock_args_api_prefix():
    ...
    try:
        sys.argv = ["magi_core-server", "--api-prefix", "/test-api"]
        args = parse_args()
        yield args
    finally:
...
class TestEnvironmentVariables:
    ...
    def test_env_api_prefix(self):
        ...
        from magi_core.api.config import get_env_value
...
class TestPathNormalization:
    ...
    def _build(self, *cli_args):
        # sys.argv must be the magi_core-server form *before* lightrag_server is
        # imported, because importing magi_core.api.utils_api evaluates
        # `global_args.whitelist_paths` at module top level, which triggers
        # parse_args() against whatever sys.argv currently holds.
        original_argv = sys.argv.copy()
        try:
            sys.argv = ["magi_core-server", *cli_args]
            from magi_core.api.config import parse_args
            from magi_core.api.lightrag_server import create_app
...
class TestRuntimeConfigInjection:
    ...
    def _build_app(self, tmp_path, monkeypatch, *cli_args):
        ...
        monkeypatch.setattr(sys, "argv", ["magi_core-server", *cli_args])
        from magi_core.api.config import parse_args
        from magi_core.api import lightrag_server
        from magi_core.api.lightrag_server import create_app
...
class TestUvicornRootPathSemantics:
    ...
    def _build_app_with_prefix(self, prefix):
        original_argv = sys.argv.copy()
        try:
            sys.argv = ["magi_core-server", "--api-prefix", prefix]
            from magi_core.api.config import parse_args
            from magi_core.api.lightrag_server import create_app
    ...
    def test_launcher_does_not_pass_root_path_to_uvicorn(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(sys, "argv", ["magi_core-server"])
        ...
        with patch("magi_core.api.lightrag_server.LightRAG") as mock_rag:
            ...
            from magi_core.api.config import parse_args, global_args as _ga
    ...
    def test_gunicorn_worker_does_not_inject_root_path(self):
        ...
        from magi_core.api import gunicorn_config
