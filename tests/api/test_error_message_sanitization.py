...
Importing ``magi_core.api.*`` eagerly instantiates ``AuthHandler`` (auth.py),
...
_SECRET = (
    "connect failed host=db.corp port=5432 user=magi_core key=/etc/magi_core/tls.key"
)
_SECRET_NEEDLES = ("db.corp", "5432", "magi_core", "/etc/magi_core/tls.key", _SECRET)
...
def _init_config(monkeypatch):
    """Initialize config with a clean argv so importing magi_core.api.* is safe.
    ...
    import magi_core.api.config as config
    ...
    try:
        from magi_core.api.config import parse_args, initialize_config
...
def _build_client(monkeypatch):
    ...
    from magi_core.api.utils_api import internal_server_error
    import magi_core.api.lightrag_server as lightrag_server
