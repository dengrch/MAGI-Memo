...
from magi_core.api.passwords import BCRYPT_PASSWORD_PREFIX, hash_password
from magi_core.tools.hash_password import main as hash_password_main
from magi_core.utils import logger as lightrag_logger
...
def auth_module(monkeypatch):
    config = import_real_api_module("magi_core.api.config")
    ...
    module = import_real_api_module("magi_core.api.auth")
    module = importlib.reload(module)
    yield module
    sys.modules.pop("magi_core.api.auth", None)
...
def test_plaintext_verify_uses_constant_time_comparison():
    ...
    """
    from magi_core.api.passwords import verify_password as verify
...
def test_invalid_auth_accounts_raises(monkeypatch):
    ...
    with pytest.raises(ValueError, match="AUTH_ACCOUNTS must use"):
        import_real_api_module("magi_core.api.auth")
...
def _load_api_key_only_modules(monkeypatch):
    ...
    auth = import_real_api_module("magi_core.api.auth")
    auth = importlib.reload(auth)
    utils_api = import_real_api_module("magi_core.api.utils_api")
...
async def test_combined_auth_rejects_guest_token_in_api_key_mode(monkeypatch):
    ...
    try:
        ...
        sys.modules.pop("magi_core.api.utils_api", None)
        sys.modules.pop("magi_core.api.auth", None)
