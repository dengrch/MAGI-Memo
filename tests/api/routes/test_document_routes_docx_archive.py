...
sys.argv = [sys.argv[0]]
_document_routes = importlib.import_module("magi_core.api.routers.document_routes")
_lightrag = importlib.import_module("magi_core.magi_core")
_pipeline = importlib.import_module("magi_core.pipeline")
_base = importlib.import_module("magi_core.base")
_constants = importlib.import_module("magi_core.constants")
_utils = importlib.import_module("magi_core.utils")
_parser_routing = importlib.import_module("magi_core.parser.routing")
_parser_registry = importlib.import_module("magi_core.parser.registry")
_parser_base = importlib.import_module("magi_core.parser.base")
...
def _ensure_shared_storage_initialized():
    ...
    shared_storage = importlib.import_module("magi_core.kg.shared_storage")
...
async def test_clear_documents_sets_and_clears_destructive_busy(tmp_path):
    ...
    class _DropSpy:
        ...
        async def drop(self):
            shared_storage_inner = importlib.import_module("magi_core.kg.shared_storage")
...
async def test_docx_archive_failure_is_best_effort(tmp_path, monkeypatch):
    ...
    import magi_core.utils_pipeline as _utils_pipeline
...
async def test_parse_native_archives_docx_after_full_docs_sync(tmp_path, monkeypatch):
    ...
    assert result["parse_format"] == "magi_core"
    ...
    assert rag.full_docs.data["doc-test"]["parse_format"] == "magi_core"
...
def test_parsed_artifact_dir_uses_unique_suffix_when_path_is_file(
    ...
    from magi_core.utils_pipeline import parsed_artifact_dir_for
...
def test_lightrag_document_reprocess_uses_full_docs_without_reparse():
    ...
    # All magi_core rows route to the internal reuse handler (reuse the stored
...
def test_default_allowlist_equals_local_engine_suffixes(tmp_path, monkeypatch):
    ...
    from magi_core.parser import registry
