...
def _isolate_env(monkeypatch):
    ...
    auth_module = sys.modules.get("magi_core.api.auth")
    if auth_module is not None:
        monkeypatch.setattr(auth_module.auth_handler, "accounts", {}, raising=False)
    utils_api_module = sys.modules.get("magi_core.api.utils_api")
...
class TestQueryStreamResponseContentType:
    ...
    def test_stream_response_has_ndjson_content_type(self):
        ...
        try:
            ...
            with patch("magi_core.api.lightrag_server.LightRAG", return_value=mock_rag):
                app = create_app(args)
...
class TestQueryStreamProtocolOrder:
    ...
    async def test_disconnect_awaits_background_query_cancellation(self):
        ...
        from magi_core.api.routers.query_routes import QueryRequest, create_query_routes
