...
``magi_core.api.config`` / ``auth`` / ``utils_api`` compute module-level state at
...
tries to open up auth by patching ``sys.modules["magi_core.api.utils_api"]``
...
_API_MODULE_PREFIX = "magi_core.api"


@pytest.fixture(autouse=True)
def _restore_lightrag_api_module_identity():
    """Restore ``magi_core.api.*`` entries a test replaced or removed.
