...
from magi_core.parser.external.docling.client import (
    CONVERT_PATH,
    POLL_PATH,
    RESULT_PATH,
    DoclingRawClient,
...
def _install_fake_httpx(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "magi_core.parser.external.docling.client.httpx.AsyncClient",
        _FakeAsyncClient,
    )
    monkeypatch.setattr(
        "magi_core.parser.external.docling.client.httpx.Timeout",
        lambda *a, **kw: None,
    )
...
async def test_docling_client_json_envelope_is_parseable_by_ir_builder(
    ...
    from magi_core.parser.external.docling.ir_builder import DoclingIRBuilder
