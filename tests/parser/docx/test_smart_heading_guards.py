...
``magi_core-download-cache --spacy --spacy-install``); they skip when the
...
def test_route_language_excludes_all_whitespace(text: str, lang: str) -> None:
    from magi_core.parser.docx.smart_heading.nlp import route_language
...
def test_strong_body_detected(text: str, expected_rule: str) -> None:
    from magi_core.parser.docx.smart_heading.guardrails import strong_body_reason
...
def test_ennum_dot_ordinal_overrides_any_ner_homophone_label(monkeypatch) -> None:
    ...
    """
    from magi_core.parser.docx.smart_heading import guardrails, nlp
...
def test_caption_prefix(text: str, vetoed: bool) -> None:
    from magi_core.parser.docx.smart_heading.guardrails import caption_prefix_reason
...
def test_imprint_marker_detected(text: str) -> None:
    from magi_core.parser.docx.smart_heading.guardrails import imprint_marker_reason
...
def test_imprint_closer_detected(text: str) -> None:
    from magi_core.parser.docx.smart_heading.guardrails import imprint_closer_reason
...
def test_is_document_date(text: str, expected: bool) -> None:
    ...
    from magi_core.parser.docx.smart_heading.guardrails import is_document_date
...
def test_is_symbolic_line(text: str, expected: bool) -> None:
    ...
    from magi_core.parser.docx.smart_heading.guardrails import is_symbolic_line
...
def test_missing_spacy_model_hard_errors(monkeypatch) -> None:
    from magi_core.parser.docx.smart_heading import nlp
    ...
    with pytest.raises(nlp.SmartHeadingNLPError, match="magi_core-download-cache"):
...
def test_missing_spacy_package_hard_errors(monkeypatch) -> None:
    ...
    with pytest.raises(nlp.SmartHeadingNLPError, match="magi_core-hku\\[api\\]"):
        nlp.sentence_count("some text")
