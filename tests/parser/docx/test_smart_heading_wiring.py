...
from magi_core.parser.base import ParseContext, ParseResult
from magi_core.parser.debug import build_debug_rag
from magi_core.parser.registry import get_parser
from magi_core.parser.routing import decode_parse_engine
...
def _parse_docx(
    ...
    *,
    ...
    """
    from magi_core.parser.docx.parser import NativeDocxParser
...
def test_wants_llm_bridge_gates_on_smart_heading() -> None:
    from magi_core.parser.docx.parser import NativeDocxParser
    from magi_core.parser.markdown.parser import NativeMarkdownParser
...
def _parse_with_stub(tmp_path, monkeypatch, stub, *, parse_engine="native"):
    ...
    rag = build_debug_rag()
    with mock.patch("magi_core.parser.docx.parse_document.extract_docx_blocks", stub):
        result = asyncio.run(
            get_parser("native").parse(
                ParseContext(rag, "doc-1", str(source_path), content_data)
            )
...
def test_smart_audit_json_is_byte_stable_across_reparse(tmp_path, monkeypatch) -> None:
    ...
    def _audit_bytes(sub: str) -> bytes:
        ...
        rag = build_debug_rag()
        with mock.patch(
            "magi_core.parser.docx.parse_document.extract_docx_blocks", stub
        ):
            asyncio.run(
                get_parser("native").parse(
                    ParseContext(
