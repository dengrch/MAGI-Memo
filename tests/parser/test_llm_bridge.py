...
from magi_core.parser.llm_bridge import (
    LLMBridgeCancelled,
    LLMBridgeShutdown,
    SyncLLMBridge,
)
...
def test_parse_result_to_dict_emits_cache_ids_only_when_present() -> None:
    ...
    base = dict(doc_id="d", file_path="f", parse_format="magi_core", content="c")
...
def test_injected_llm_reaches_extract_and_is_callable(tmp_path, monkeypatch) -> None:
    ...
    with (
        ...
        mock.patch(
            "magi_core.parser.docx.parse_document.extract_docx_blocks", _stub_blocks
        ),
    ):
        result = asyncio.run(
