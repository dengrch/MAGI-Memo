...
``delete_by_entity`` …) all delegate to :func:`magi_core.magi_core._run_sync`,
...
* Driven from a different — but still alive — loop, e.g.
  ...
  the shared ``asyncio.Lock`` objects in ``magi_core.kg.shared_storage`` to the
...
from magi_core.magi_core import _run_sync
