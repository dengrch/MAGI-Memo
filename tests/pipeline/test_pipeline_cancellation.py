...
from magi_core.exceptions import MultimodalAnalysisError, PipelineCancelledException
...
from magi_core.parser.llm_bridge import LLMBridgePipelineCancelled, SyncLLMBridge
...
async def _shutdown_role_workers(rag: LightRAG) -> None:
    ...
    for func in rag.role_llm_funcs.values():
        try:
            ...
            logging.getLogger("magi_core").warning(
...
async def test_parse_worker_drains_queue_when_cancelled_before_start(
    ...
    try:
        ...
        monkeypatch.setattr("magi_core.pipeline.get_parser", get_parser_spy)
...
async def test_pipeline_cancel_interrupts_inflight_native_parser_llm(
    ...
    try:
        ...
        monkeypatch.setattr(
            "magi_core.pipeline.get_parser", lambda *_a, **_k: _BlockingNativeParser()
        )
