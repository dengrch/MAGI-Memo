...
backend flush is exercised — this is pure magi_core.py logic.
"""
...
def _propagate_lightrag_logs():
    """The ``magi_core`` logger sets ``propagate=False``, so caplog's root
...
async def test_insert_done_multiple_failures_raises_first_logs_rest(
    ...
    try:
        ...
        with caplog.at_level("ERROR", logger="magi_core"):
