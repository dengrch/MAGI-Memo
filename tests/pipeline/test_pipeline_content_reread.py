...
from magi_core.kg.shared_storage import get_namespace_data, get_namespace_lock
from magi_core.pipeline import _BatchRunContext
from magi_core.parser.base import ParseResult
from magi_core.parser.registry import parser_specs_snapshot
from magi_core.utils import EmbeddingFunc, Tokenizer, get_content_summary
...
async def test_parse_worker_flushes_smart_heading_cache_before_handoff(
    ...
    try:
        ...
        monkeypatch.setattr("magi_core.pipeline.get_parser", lambda *_a, **_k: _Parser())
        await ctx.parse_queues["native"].put(
...
async def test_process_strips_lightrag_marker_on_reread(tmp_path):
    """For a magi_core-format record, the re-read must strip the ``{{LRdoc}}``
    ...
    try:
        ...
        await rag.full_docs.upsert(
            {
                doc_id: {
                    # Stored WITH the marker, as parse persists magi_core docs.
