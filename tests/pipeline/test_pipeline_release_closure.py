...
from magi_core import LightRAG, ROLES, RoleLLMConfig
from magi_core.base import DocStatus
...
from magi_core.parser.registry import get_parser
...
def test_filename_hint_missing_required_endpoint_rejects(monkeypatch):
    from magi_core.parser.routing import resolve_file_parser_directives
...
def test_validate_process_options_rejects_invalid_combos():
    from magi_core.parser.routing import validate_process_options
...
def test_doc_status_metadata_carry_over_helper():
    ...
    """
    from magi_core.utils_pipeline import doc_status_transition_metadata
...
def test_carry_over_keys_grouped_by_stage():
    ...
    """
    from magi_core.utils_pipeline import _DOC_STATUS_METADATA_CARRY_OVER_KEYS
...
def test_acquire_failure_does_not_stamp_stopped_bookkeeping(tmp_path, monkeypatch):
    ...
    async def _run():
        import magi_core.pipeline as pipeline_mod
        from magi_core.kg.shared_storage import get_namespace_data
...
def test_enqueue_publish_failure_falls_back_to_auto_rescan(tmp_path, monkeypatch):
    ...
    async def _run():
        from magi_core.kg.shared_storage import get_pipeline_ingress
...
def test_enqueue_status_upsert_failure_survives_wake_failure(
    ...
    async def _run():
        ...
        try:
            ...
            # The "magi_core" logger sets propagate=False, so caplog (a root
            # handler) cannot see its records unless we re-enable propagation
            # for the duration of the test (monkeypatch auto-reverts it).
            monkeypatch.setattr(logging.getLogger("magi_core"), "propagate", True)
...
def test_decide_pipeline_next_step_atomic_handoff(tmp_path):
    ...
    async def _run():
        ...
        from magi_core.pipeline import PipelineNextStep
...
def test_concurrent_enqueue_dedupes_same_content_different_filenames(tmp_path):
    ...
    async def _run():
        import magi_core.pipeline as pipeline_module
...
def test_enqueue_rejects_removed_or_unknown_docs_format(tmp_path):
    """The 'magi_core' ingestion entrypoint was removed: enqueue accepts only
    ...
    async def _run():
        ...
        try:
            with pytest.raises(ValueError, match="Unsupported docs_format"):
                await rag.apipeline_enqueue_documents(
                    "",
                    file_paths="first.magi_core",
                    docs_format="magi_core",
                )
            with pytest.raises(ValueError, match="Unsupported docs_format"):
                await rag.apipeline_enqueue_documents(
                    "some content",
...
def test_pending_parse_duplicate_hash_fails_and_archives_source(tmp_path, monkeypatch):
    ...
    async def _run():
        ...
        try:
            ...
            import magi_core.magi_core as lightrag_module
            import magi_core.pipeline as pipeline_module
            ...
            monkeypatch.setattr(
                "magi_core.parser.docx.parse_document.extract_docx_blocks",
...
def test_three_phase_status_flow(tmp_path, monkeypatch):
    async def _run():
        ...
        monkeypatch.setattr("magi_core.pipeline.merge_nodes_and_edges", _fake_merge)
...
def test_analyze_multimodal_invalid_json_hard_fails(tmp_path):
    ...
    async def _run():
        ...
        from magi_core.exceptions import MultimodalAnalysisError
...
def test_parse_mineru_to_lightrag_document(tmp_path, monkeypatch):
    ...
    """
    from magi_core.parser.external.mineru import compute_size_and_hash
    from magi_core.parser.external.mineru.cache import current_mineru_options_signature
    from magi_core.parser.external.mineru.client import MinerURawClient
    from magi_core.parser.external.mineru.manifest import (
        Manifest,
        ManifestFile,
        write_manifest,
    )
    ...
    async def _run():
        ...
        assert parsed["parse_format"] == "magi_core"
        ...
        assert full_doc["parse_format"] == "magi_core"
...
def test_build_chunks_dict_preserves_existing_llm_cache_list():
    ...
    from magi_core.utils_pipeline import build_chunks_dict_from_chunking_result
...
def test_engine_params_survive_persist_to_full_docs(tmp_path, monkeypatch):
    ...
    """
    from magi_core.parser.external.mineru import compute_size_and_hash
    from magi_core.parser.external.mineru.cache import (
