...
from magi_core.utils import EmbeddingFunc, Tokenizer
...
async def test_vlm_call_carries_image_inputs(tmp_path):
    ...
    the mock — see ``priority_limit_async_func_call`` in magi_core.utils."""
    call_log: list[dict] = []
    rag = _build_rag(
    ...
    try:
        ...
        # _priority is consumed by the wrapper (see magi_core.utils
...
async def test_analyze_worker_marks_doc_failed_on_multimodal_error(tmp_path):
    ...
    from magi_core.base import DocProcessingStatus, DocStatus
    from magi_core.pipeline import _BatchRunContext
    from magi_core.parser.registry import parser_specs_snapshot
...
async def test_oversized_table_content_truncated_to_extract_budget(tmp_path):
    ...
    """
    from magi_core.constants import DEFAULT_MAX_EXTRACT_INPUT_TOKENS
...
async def test_max_extract_input_tokens_env_var_lowers_cap_and_logs_warning(
    ...
    try:
        ...
        with caplog.at_level(logging.WARNING, logger="magi_core.pipeline"):
            await rag.analyze_multimodal(
                doc_id="doc-mid",
                file_path="fixture.pdf",
                parsed_data={"blocks_path": str(blocks_path)},
...
def test_structurally_starved_config_warns_once(caplog, _propagate_lightrag_logger):
    ...
    from magi_core.pipeline import _warn_content_budget_structurally_starved
...
def test_table_content_format_label_html_and_json():
    from magi_core.prompt_multimodal import table_content_format_label
...
async def test_truncated_vlm_response_not_cached_and_recomputed(tmp_path):
    ...
    """
    from magi_core.utils import TruncatedResponse
