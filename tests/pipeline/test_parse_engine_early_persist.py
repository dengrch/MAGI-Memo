...
from magi_core.utils import EmbeddingFunc, Tokenizer, compute_mdhash_id
...
def test_resolve_engine_format_based_fallback():
    # No explicit engine: structured magi_core → native, everything else
...
def test_lightrag_doc_records_native_engine_at_parsing(tmp_path):
    """A magi_core-format doc on resume (already-parsed full_docs row pulled
    ...
    The row is seeded directly (full_docs ``parse_format=magi_core`` +
    PENDING doc_status), mirroring how such rows exist in production now
    that the ``docs_format="magi_core"`` enqueue entrypoint is removed:
