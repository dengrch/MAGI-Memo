...
import magi_core.utils as utils_module
from magi_core.kg.postgres_impl import PGGraphStorage, PostgreSQLDB
...
async def test_execute_timing_logs_success():
    ...
    with patch("magi_core.kg.postgres_impl.performance_timing_log") as timing_log:
...
def _read_performance_timing_flag() -> bool:
    ...
    ``magi_core.utils`` to a brand-new object in the shared, in-place module,
    silently breaking ``isinstance`` identity for the many modules that do
    ``from magi_core.utils import X`` — most visibly the LLM providers' shared
    ...
    env parsing while leaving ``sys.modules['magi_core.utils']`` — and everyone's
    by-value imports — untouched.
    """
    spec = importlib.util.spec_from_file_location(
        "magi_core.utils", utils_module.__file__
    )
