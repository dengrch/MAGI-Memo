...
from magi_core.kg.postgres_impl import PGDocStatusStorage, PGKVStorage, PGVectorStorage
...
async def test_upsert_full_docs_tuple_order():
    ...
    data = {
        "doc-1": {
            ...
            "sidecar_location": "magi_core://sidecar/doc-1",
            "parse_format": "magi_core",
    ...
    assert row[4] == "magi_core://sidecar/doc-1"
    assert row[5] == "magi_core"
...
async def test_upsert_full_docs_missing_pipeline_fields_pass_through_as_none():
    ...
    blow away a previously-set 'magi_core').
    """
    storage = make_storage(NameSpace.KV_STORE_FULL_DOCS)
