...
from magi_core.kg.opensearch_impl import (
    OpenSearchKVStorage,
...
from magi_core.base import DocStatus, DocProcessingStatus
...
def patch_data_init_lock():
    ...
    with patch(
        "magi_core.kg.opensearch_impl.get_data_init_lock", side_effect=_mock_lock_factory
    ):
        yield
...
def patch_namespace_lock():
    ...
    with patch("magi_core.kg.opensearch_impl.get_namespace_lock", side_effect=factory):
        yield
...
def patch_shard_doc_supported():
    ...
    """
    with patch("magi_core.kg.opensearch_impl._shard_doc_supported", True):
        yield
...
class TestBulkBatchLimits:
    ...
    def test_resolve_non_positive_disables_and_warns(self):
        ...
        with patch.dict("os.environ", env, clear=True):
            with patch("magi_core.kg.opensearch_impl.logger") as mock_logger:
                payload, upserts, deletes = _resolve_bulk_batch_limits()
    ...
    async def test_run_chunked_empty_short_circuits(self):
        with patch(
            "magi_core.kg.opensearch_impl.helpers.async_bulk", new_callable=AsyncMock
        ) as mock_bulk:
...
class TestClientManager:
    ...
    async def test_singleton_and_refcount(self):
        ...
        with patch("magi_core.kg.opensearch_impl.AsyncOpenSearch") as mock_cls:
...
class TestMirroredIdVerification:
    ...
    async def test_passes_when_mapping_present(self, mock_client):
        ...
        with patch("magi_core.kg.opensearch_impl._shard_doc_supported", False):
...
class TestKVStorage:
    ...
    async def test_initialize_fails_on_legacy_index_without_mirrored_id(
        ...
        with (
            ...
            patch("magi_core.kg.opensearch_impl._shard_doc_supported", False),
        ):
            s = self._make(global_config, embed_func)
            with pytest.raises(RuntimeError, match="__mirrored_id"):
                await s.initialize()
...
class TestKVStorageBatching:
    ...
    async def test_kv_finalize_raises_when_retryable_buffer_remains(
        ...
        with patch.object(ClientManager, "get_client", return_value=mock_client):
            with patch.object(
                ...
                with patch(
                    "magi_core.kg.opensearch_impl.helpers.async_bulk",
    ...
    async def test_kv_drop_discards_buffers_and_serialises_with_flush(
        ...
        with patch.object(ClientManager, "get_client", return_value=mock_client):
            with patch("magi_core.kg.opensearch_impl.helpers.async_bulk", new=slow_bulk):
                s = self._make(global_config, embed_func)
                await s.initialize()
                await s.upsert({"k1": {"content": "x"}})
                await s.delete(["k2"])
...
class TestGraphStorage:
    ...
    async def test_migrate_edges_logs_progress_for_large_scan(
        ...
        with (
            patch("magi_core.kg.opensearch_impl._EDGE_MIGRATION_PROGRESS_INTERVAL", 2),
            patch(
                "magi_core.kg.opensearch_impl.helpers.async_bulk",
                new=AsyncMock(return_value=(0, [])),
            ),
            patch("magi_core.kg.opensearch_impl.logger", fake_logger),
        ):
            await s._migrate_edges_to_canonical_id_if_needed()
...
class TestVectorStorageBatching:
    ...
    def test_extract_bulk_failed_ids_classifies_by_status(self):
        from magi_core.kg.opensearch_impl import _extract_bulk_failed_ids
...
class TestEmbeddingBatchNumDiagnosis:
    ...
    (magi_core/operate.py). ``EMBEDDING_BATCH_NUM`` only slices the items
