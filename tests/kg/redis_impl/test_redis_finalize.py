...
from magi_core.kg.redis_impl import (
...
async def test_release_pool_closes_only_on_last_reference():
    ...
    try:
        with patch(
            "magi_core.kg.redis_impl.ConnectionPool.from_url", return_value=fake_pool
        ):
...
def test_init_error_no_running_loop_does_not_mask_exception(cls):
    ...
    try:
        with warnings.catch_warnings(record=True) as caught:
            ...
            with (
                patch(
                    "magi_core.kg.redis_impl.ConnectionPool.from_url",
                    return_value=fake_pool,
                ),
                patch("magi_core.kg.redis_impl.Redis", side_effect=boom),
            ):
