...
prompt_with_default() {{
  case "$1" in
    ...
    "PostgreSQL user") printf 'magi_core' ;;
    "PostgreSQL database") printf 'magi_core' ;;
    *) printf '%s' "$2" ;;
  esac
}}
...
""")
    ...
    assert values["POSTGRES_DATABASE"] == "magi_core"
    assert (
        values["PROMPT_LOG"]
        == "PostgreSQL host[localhost]|PostgreSQL user[rag]|secret:PostgreSQL password: [rag]|PostgreSQL database[magi_core]"
    )
...
""")
    ...
    assert (
        values["PROMPT_LOG"]
        == "PostgreSQL host[localhost]|PostgreSQL port[5432]|PostgreSQL user[rag]|PostgreSQL password: [rag]|PostgreSQL database[magi_core]"
    )
...
prompt_with_default() {{
  case "$1" in
    ...
    "Milvus database name") printf 'magi_core' ;;
    *) printf '%s' "$2" ;;
  esac
}}
...
""")
    assert values["MILVUS_DB_NAME"] == "magi_core"
