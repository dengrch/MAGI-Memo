...
def test_find_generated_compose_file_prefers_final_compose_file(tmp_path: Path) -> None:
    ...
    write_text_lines(
        tmp_path / "docker-compose.final.yml",
        ["services:", "  magi_core:", "    image: final/magi_core"],
    )
    write_text_lines(
        tmp_path / "docker-compose.development.yml",
        ["services:", "  magi_core:", "    image: dev/magi_core"],
    )
    write_text_lines(
        tmp_path / "docker-compose.production.yml",
        ["services:", "  magi_core:", "    image: prod/magi_core"],
    )
    output = run_bash(f"""
set -euo pipefail
source "{REPO_ROOT}/scripts/setup/setup.sh"
...
ORIGINAL_ENV_VALUES[POSTGRES_DATABASE]="magi_core"
    ...
    [
        ...
        (
            ...
            [
                ...
                "POSTGRES_USER=magi_core",
                "POSTGRES_PASSWORD=secret",
                "POSTGRES_DATABASE=magi_core",
            ],
...
def test_backup_only_backs_up_env_and_generated_compose(tmp_path: Path) -> None:
    ...
    compose_content = (
        "\n".join(["services:", "  magi_core:", "    image: example/magi_core:test"])
        + "\n"
    )
    write_text_lines(tmp_path / ".env", ["HOST=0.0.0.0"])
