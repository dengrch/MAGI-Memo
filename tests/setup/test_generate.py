...
def test_generate_docker_compose_removes_lightrag_container_name_from_existing_output(
    ...
    """Compose regeneration should strip fixed magi_core container names from prior output."""
    write_text_lines(
...
def test_generate_docker_compose_injects_healthchecks_and_lightrag_depends_on(
    ...
    write_text_lines(
        tmp_path / "docker-compose.yml",
        ["services:", "  magi_core:", "    image: example/magi_core:test"],
    )
    write_text_lines(
        tmp_path / "env.example",
        (REPO_ROOT / "env.example").read_text(encoding="utf-8").splitlines(),
...
""")
    ...
    lightrag_start = generated_compose.index("  magi_core:\n")
...
def test_generate_docker_compose_repairs_misplaced_lightrag_depends_on_from_existing_output(
    ...
    """Regeneration should move stale magi_core depends_on content back onto the magi_core service."""
    write_text_lines(
        tmp_path / "docker-compose.final.yml",
        [
            "services:",
...
def test_generate_docker_compose_normalizes_lightrag_restart_policy_from_existing_output(
    ...
    """Regeneration should replace legacy magi_core restart with deploy.restart_policy."""
    write_text_lines(
        tmp_path / "docker-compose.final.yml",
        [
            "services:",
...
ENV_VALUES[POSTGRES_USER]="magi_core"
ENV_VALUES[POSTGRES_PASSWORD]="secret"
ENV_VALUES[POSTGRES_DATABASE]="magi_core"
add_docker_service "postgres"
...
def test_generate_env_file_avoids_double_quotes_for_compose_sensitive_strings(
    ...
    env_example.write_text(
        "\n".join(
            [
                "WEBUI_TITLE='MAGI Memo'",
                "WEBUI_DESCRIPTION='Episode and Atom Memory Core'",
...
ENV_VALUES[WEBUI_TITLE]='MAGI Memo'
ENV_VALUES[WEBUI_DESCRIPTION]='Episode and Atom Memory Core'
...
""")
    generated_lines = (tmp_path / ".env").read_text(encoding="utf-8").splitlines()
    assert "WEBUI_TITLE='MAGI Memo'" in generated_lines
    assert (
        "WEBUI_DESCRIPTION='Episode and Atom Memory Core'" in generated_lines
    )
...
def test_generate_docker_compose_injects_env_overrides_into_lightrag_not_after_managed_services(
    ...
    """Env overrides must appear inside the magi_core environment block, not after managed services.
    ...
    the magi_core service once postgres/neo4j were merged in.
    """
    compose_file = tmp_path / "docker-compose.yml"
    compose_file.write_text(
        "\n".join(
...
""")
    ...
    lightrag_pos = result.index("  magi_core:")
