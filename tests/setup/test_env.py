...
def test_env_base_flow_backs_up_legacy_generated_compose_before_rewrite(
    ...
    legacy_compose = (
        "\n".join(["services:", "  magi_core:", "    image: prod/magi_core"]) + "\n"
    )
    write_text_lines(
        tmp_path / "env.example",
        (REPO_ROOT / "env.example").read_text(encoding="utf-8").splitlines(),
...
""")
        ...
        assert "  magi_core:" in generated_compose
...
""",
    ...
    assert "POSTGRES_DATABASE=magi_core" in generated_env
    assert 'POSTGRES_USER: "rag"' in generated_compose
    assert 'POSTGRES_PASSWORD: "rag"' in generated_compose
    assert 'POSTGRES_DB: "magi_core"' in generated_compose
