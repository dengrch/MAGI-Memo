...
from magi_core.base import SourceConflict, SourceUnique
from magi_core.kg.json_doc_status_impl import JsonDocStatusStorage
...
async def test_commit_holds_the_enqueue_serialize_lock(tmp_path):
    ...
    from magi_core.constants import ENQUEUE_SERIALIZE_LOCK_NAMESPACE
    from magi_core.kg.shared_storage import get_namespace_lock
...
async def test_a_destructive_job_in_flight_refuses_the_commit(tmp_path):
    ...
    from magi_core.exceptions import StorageControlPlaneError
    from magi_core.kg.shared_storage import (
        get_namespace_data,
        get_namespace_lock,
        initialize_pipeline_status,
...
async def test_a_contentless_primary_is_refused_offline_too(tmp_path):
    ...
    from magi_core.exceptions import SourceConflictPrimaryUnusableError
...
async def test_the_cli_refuses_to_apply_when_full_docs_cannot_be_opened(
    ...
    import magi_core
    from magi_core.tools import source_conflict_repair as tool
    ...
    monkeypatch.setattr(magi_core, "LightRAG", _FakeRAG)
...
async def test_the_absent_recovery_hint_matches_what_actually_happened(tmp_path):
    ...
    from magi_core.exceptions import StorageControlPlaneError
    from magi_core.tools.source_conflict_repair import verify_repair_outcome
