...
def _hermetic_mineru_env(monkeypatch):
    ...
    ``magi_core/api/{auth,config}.py`` call ``load_dotenv(override=False)``
...
_SPACY_DOWNLOAD_HINT = "magi_core-download-cache --spacy --spacy-install"
...
def pytest_configure(config):
    ...
    try:
        from magi_core.parser.routing import smart_heading_default_enabled
