...
from magi_core.chunker.paragraph_semantic import (
...
def _propagate_lightrag_logger(monkeypatch):
    # The ``magi_core`` logger sets ``propagate=False`` (see magi_core.utils), so
...
def test_extract_table_id_variants():
    from magi_core.table_markup import extract_table_id
...
def test_inject_header_into_table_slice_json_and_html():
    from magi_core.chunker.paragraph_semantic import _inject_header_into_table_slice
...
def test_split_table_text_multi_row_header_pinned_not_duplicated():
    ...
    from magi_core.table_markup import TABLE_TAG_RE
