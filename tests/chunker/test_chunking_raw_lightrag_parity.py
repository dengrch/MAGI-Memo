"""F-chunking parity between raw and magi_core formats.

After the F-chunking unification, ``apipeline_process_enqueue_documents``
strips the ``{{LRdoc}}`` marker from magi_core-format content and feeds the
...
* T1: identical input text produces identical chunking inputs whether it
  arrives as raw or as an already-parsed magi_core full_docs row pulled
  back through the parse queue on resume (ReuseParser) — the only
  production path for magi_core rows now that the ``docs_format="magi_core"``
...
* T3: ``extraction_meta["parse_format"]`` (surfaced via
  ``doc_status.metadata``) is ``"magi_core"`` for natively-parsed docs —
...
* T6: a ``pending_parse`` document that resolves to magi_core at parse
...
from magi_core.utils_pipeline import make_lightrag_doc_content
...
# ---------------------------------------------------------------------------
# T1 — parity: raw vs magi_core produce identical chunking input
# ---------------------------------------------------------------------------


async def _seed_lightrag_row(rag: LightRAG, doc_id: str, *, body: str, file_path: str):
    """Seed an already-parsed magi_core full_docs row + a PENDING doc_status.

    Mirrors how magi_core-format rows exist in production: written by the
...
def test_chunking_input_parity_raw_vs_lightrag(tmp_path):
    ...
    whether it arrives as raw or as a resumed magi_core full_docs row."""
    ...
    async def _run():
        ...
        assert spy_lr["calls"] >= 1, "magi_core doc never reached chunking_func"
        assert spy_lr["input"] == spy_raw["input"] == expected_merged, (
            "chunking_func received different inputs for raw vs magi_core; "
...
def _stub_docx_blocks(monkeypatch, body_paragraphs: list[str]) -> None:
    ...
    persists the magi_core-format full_docs row."""
...
class _ListHandler(logging.Handler):
    ...
    The ``magi_core`` logger has ``propagate = False`` so pytest's caplog
...
def test_explicit_R_dispatches_to_recursive_character(tmp_path, monkeypatch):
    ...
    import magi_core.chunker as chunker_pkg
    from magi_core.chunker import chunking_by_recursive_character as real_r
    ...
    # The dispatcher does ``from magi_core.chunker import …`` inside the
    ...
    async def _run():
        ...
        lightrag_logger = logging.getLogger("magi_core")
...
# ---------------------------------------------------------------------------
# T6 — pending_parse → magi_core summary is populated after PROCESSED
# ---------------------------------------------------------------------------
...
def test_raw_text_starting_with_marker_chunked_verbatim(tmp_path):
    ...
    characters.  ``strip_lightrag_doc_prefix`` is a magi_core-only contract
