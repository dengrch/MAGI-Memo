...
from magi_core.utils import EmbeddingFunc, Tokenizer, priority_limit_async_func_call
...
def lightrag_logger_propagating(monkeypatch):
    """Force the magi_core logger to propagate so caplog can capture records."""
...
def test_role_max_async_defaults_inherit_base(tmp_path, monkeypatch):
    ...
    # (e.g. magi_core.kg.networkx_impl) are imported lazily during
...
def test_role_llm_config_logs_once_on_init_with_metadata(
    ...
    with caplog.at_level("INFO", logger="magi_core"):
        rag = _make_rag(
            tmp_path,
            role_llm_configs={
                "query": RoleLLMConfig(
