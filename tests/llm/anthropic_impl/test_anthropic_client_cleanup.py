...
from magi_core.llm.anthropic import anthropic_complete_if_cache
...
async def test_client_closed_on_rate_limit_error():
    ...
    with (
        patch("magi_core.llm.anthropic.AsyncAnthropic", return_value=fake_client),
...
async def test_client_closed_on_api_timeout_error():
    ...
    with (
        patch("magi_core.llm.anthropic.AsyncAnthropic", return_value=fake_client),
        patch("magi_core.llm.anthropic.logger") as mock_logger,
...
async def test_client_closed_after_non_streaming_success():
    ...
    with patch("magi_core.llm.anthropic.AsyncAnthropic", return_value=fake_client):
