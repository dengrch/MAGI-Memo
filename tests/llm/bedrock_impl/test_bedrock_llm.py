...
from magi_core.llm.bedrock import (
    bedrock_complete,
    bedrock_complete_if_cache,
    bedrock_embed,
)
from magi_core.llm_roles import ROLES
...
def _reload_api_modules_if_mocked() -> None:
    """Drop cached magi_core.api entries so importlib reloads with isolated env.

    Other test files (e.g. test_token_auto_renewal.py) replace
    ``sys.modules["magi_core.api.config"]`` with a Mock at import time. When
    ...
    """
    for modname in (
        "magi_core.api.lightrag_server",
        "magi_core.api.utils_api",
        "magi_core.api.auth",
        "magi_core.api.config",
    ):
...
async def test_bedrock_complete_skips_reasoning_content_block(monkeypatch):
    ...
    with patch(
        "magi_core.llm.bedrock.aioboto3.Session",
...
async def test_bedrock_complete_forwards_keyword_extraction_to_if_cache():
    ...
    with patch(
        "magi_core.llm.bedrock.bedrock_complete_if_cache",
...
def _make_args(tmp_path):
    ...
    overrides = dict(
        ...
        simulated_model_name="magi_core",
...
async def test_create_app_query_role_uses_bedrock_binding(tmp_path, monkeypatch):
    ...
    config = importlib.import_module("magi_core.api.config")
    config.initialize_config(_make_args(tmp_path), force=True)
    lightrag_server = importlib.import_module("magi_core.api.lightrag_server")
    ...
    with (
        ...
        patch(
            "magi_core.llm.openai.openai_complete_if_cache",
...
async def test_create_app_keyword_openai_role_forwards_nested_extra_body(
    ...
    with (
        caplog.at_level("INFO", logger="magi_core"),
        patch(
            "magi_core.llm.openai.openai_complete_if_cache",
            AsyncMock(
...
async def test_bedrock_max_tokens_stop_reason_marks_result_truncated(monkeypatch):
    ...
    from magi_core.utils import is_truncated_response
