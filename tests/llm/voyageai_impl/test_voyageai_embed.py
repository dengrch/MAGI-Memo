"""Unit tests for magi_core.llm.voyageai.
...
def patched_async_client(fake_voyage_response):
    ...
    with patch(
        "magi_core.llm.voyageai.voyageai.AsyncClient", return_value=fake_client
    ) as m:
        yield captured, m
...
def test_anthropic_embed_deprecation_shim():
    ...
    from magi_core.llm.anthropic import anthropic_embed  # must not ImportError
