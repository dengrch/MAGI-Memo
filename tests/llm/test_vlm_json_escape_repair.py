...
from magi_core.utils import repair_vlm_json_escape_damage


@pytest.fixture
def _propagate_lightrag_logger(monkeypatch):
    """``magi_core.utils.logger`` sets ``propagate = False``; restore
...
def test_prompts_require_double_escaped_backslashes():
    ...
    from magi_core.prompt_multimodal import MULTIMODAL_PROMPTS
...
async def test_extraction_json_result_repairs_latex_escape_damage():
    ...
    from magi_core.operate import _process_json_extraction_result
