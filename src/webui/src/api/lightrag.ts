...
export type MemoryOverview = {
  ...
  episodes: number
  episode_statuses: Record<string, number>
  atoms: number
  atom_statuses: Record<string, number>
  evidence: number
  entities: number
  relations: number
  communities: number
...
export const getMemoryOverview = async (): Promise<MemoryOverview> => {
  const response = await axiosInstance.get('/memory/overview')
  return response.data
}

export const getMemoryEpisodes = async (params: {
  page?: number
  page_size?: number
  status?: string
  kind?: string
...
export const getMemoryEpisode = async (episodeId: string): Promise<MemoryEpisode> => {
  const response = await axiosInstance.get(`/memory/episodes/${encodeURIComponent(episodeId)}`)
  return response.data
}

export const getMemoryAtoms = async (params: {
  page?: number
  page_size?: number
  owner_type?: 'entity' | 'relation'
  temporal_status?: 'pending' | 'active' | 'invalid' | 'expired'
