...
import { ActivityIcon, ChevronDownIcon, HistoryIcon, RouteIcon } from 'lucide-react'
...
function ExplorationProperties() {
  ...
  return (
    <div className={cn(graphGlassPanelClass, 'flex max-h-[calc(100vh-7rem)] w-80 flex-col overflow-hidden text-xs')}>
      <div className="border-border border-b px-3 py-2">
        <div className="text-muted-foreground text-[10px] font-semibold uppercase tracking-wider">
          {isNode ? 'node' : 'edge'} · read only
        </div>
        <div className="mt-0.5 truncate text-sm font-medium" title={title}>{title}</div>
      </div>
    ...
    <>
      <Popover>
        <PopoverTrigger asChild>
          <button
            type="button"
            className={cn(graphGlassControlClass, 'absolute top-2 left-1/2 z-20 flex max-w-[42rem] -translate-x-1/2 items-center gap-2 px-3 py-2')}
          >
            <RouteIcon
              className="size-3.5 shrink-0"
              style={{ color: EXPLORATION_TRACE_COLOR }}
            />
            <span className="truncate text-xs" title={query}>{query}</span>
        ...
        <PopoverContent
          ...
          side="bottom"
          sideOffset={6}
          className={cn(graphGlassPanelClass, 'data-[state=open]:slide-in-from-top-3 data-[state=closed]:slide-out-to-top-2 data-[state=open]:zoom-in-100 data-[state=closed]:zoom-out-100 max-h-[min(32rem,70vh)] w-[min(34rem,calc(100vw-2rem))] origin-top overflow-hidden p-0 duration-200')}
        >
          <div className="border-border flex items-center justify-between border-b px-3 py-2.5">
            <div className="min-w-0">
              ...
              <div className="mt-1 truncate text-xs" title={query}>{query}</div>
            </div>
            {replaySeq !== null && (
              <button
                type="button"
                className="hover:bg-accent ml-3 shrink-0 rounded-lg border border-border/60 px-2 py-1 text-[10px]"
              ...
              return (
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    ...
                    <span
                      className="text-muted-foreground mt-0.5 block truncate text-[10px]"
                      title={`${frame.agentId} · ${frame.query ?? query}`}
                    >
      ...
      <div className={cn(graphGlassPanelClass, 'absolute top-14 left-2 z-10 w-60 px-3 py-2')}>
        ...
        <div className="space-y-1.5">
          {frames.map((frame) => {
            ...
            const agentQuery = frame.query ?? query
            return (
              <div key={frame.agentId} className="flex min-w-0 items-center gap-2 text-xs">
                <span
                  className="size-2.5 shrink-0 rounded-full border-2"
                  style={{ borderColor: color, boxShadow: `0 0 8px 2px ${color}99` }}
                />
                <span className="shrink-0 font-mono text-[10px]" title={frame.agentId}>
                  {frame.agentId.slice(0, 3)}
                </span>
                <span className="text-muted-foreground/80 min-w-0 flex-1 truncate text-[10px]" title={agentQuery}>
                  {agentQuery}
                </span>
              </div>
            )
        ...
        <div className="text-muted-foreground mt-2 border-t border-border/60 pt-2 text-[10px]">
          Dispatch agent: <span className="font-mono">{shortId(explorationId)}</span>
        </div>
      </div>
    </>
  )
...
export default function MelchiorView() {
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      ...
      try {
        ...
        if (!cancelled) {
          useExplorationStore.getState().setError(isNetworkError(cause)
            ? null
  ...
  useEffect(() => {
    if (currentTab !== 'melchior' || currentExplorationId === null) return
    let cancelled = false
    let pending = false
    const refresh = async () => {
      if (pending) return
