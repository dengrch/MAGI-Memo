...
from magi_core.exceptions import (
    IndexFlushError,
...
def _get_relationship_vdb_timeout_seconds(global_config: dict[str, Any]) -> float:
    ...
    Rationale:
    - Neo4j edge writes and vector writes both perform remote I/O.
    - Vector upserts additionally perform embedding calls, so they need a
      defensive timeout derived from the embedding provider timeout.
    """
...
_MAGI_ATOM_CITATION_PATTERN = re.compile(r"\[atom-[0-9A-Za-z_-]+\]\s*")


def _truncate_vdb_content(content: str, global_config: dict, content_label: str) -> str:
    """Prepare vector-store text and clamp it below embedding limits.

    Graph descriptions retain Atom citations for provenance. The vector text
    deliberately removes only the synthetic ``[atom-id]`` tokens; temporal
    status and the fact text remain available to retrieval.
    """

    if not content:
        return content

    if global_config.get("magi_memory_enabled", False):
        content = _MAGI_ATOM_CITATION_PATTERN.sub("", content)
...
async def _process_json_extraction_result(
    ...
    timestamp: int,
    file_path: str = "unknown_source",
    magi_memory_enabled: bool = False,
) -> tuple[dict, dict]:
        ...
        try:
            ...
            if isinstance(raw_aliases, list):
                for raw_alias in raw_aliases:
                    alias = sanitize_and_normalize_extracted_text(
                        str(raw_alias), remove_inner_quotes=True
                    )
                    if alias and alias != entity_name and alias not in aliases:
            ...
            if magi_memory_enabled:
                ...
                atom_payloads = [{"content": entity_data.get("description", "")}]

            for atom_payload in atom_payloads:
                if not isinstance(atom_payload, dict):
                    continue
                atom_content = sanitize_and_normalize_extracted_text(
                    str(atom_payload.get("content", ""))
                )
                if not atom_content.strip():
                    logger.warning(
                        f"{chunk_key}: Empty Atom content for entity '{entity_name}'"
                    )
                    continue
                node_data = dict(
                    entity_name=truncated_name,
                    entity_type=entity_type,
                    aliases=aliases,
                    description=atom_content,
                    source_id=chunk_key,
                    file_path=file_path,
                    timestamp=timestamp,
                    atom_payload=(dict(atom_payload) if magi_memory_enabled else None),
                )
    ...
    for rel_data in relationships_list:
        ...
        try:
            ...
            if magi_memory_enabled and (
                truncated_source not in maybe_nodes
                or truncated_target not in maybe_nodes
            ):
                logger.warning(
            ...
            if magi_memory_enabled:
                atom_payloads = rel_data.get("atoms", [])
                if not isinstance(atom_payloads, list) or not atom_payloads:
                    logger.warning(
                        f"{chunk_key}: MAGI relation '{source}' ~ '{target}' has no atoms"
                    )
                    continue
            else:
                atom_payloads = [{"content": rel_data.get("description", "")}]
            ...
            for atom_payload in atom_payloads:
                ...
                if not atom_content.strip():
                    logger.warning(
                        f"{chunk_key}: Empty Atom content for relationship "
                        f"'{source}' ~ '{target}'"
                    )
                    continue
                edge_data = dict(
...
async def _merge_nodes_then_upsert(
    ...
    try:
        ...
        if already_node:
            ...
            if existing_desc and not global_config.get(
                "magi_projection_replace", False
            ):
        ...
        # MAGI keeps every Atom occurrence in SQLite insertion order. Legacy
        # LightRAG keeps its original same-document description deduplication.
        if global_config.get("magi_preserve_atom_order", False):
            sorted_nodes = sorted(
                (dp for dp in nodes_data if dp.get("description")),
                key=lambda item: item.get("magi_atom_order", 0),
            )
        else:
            unique_nodes = {}
        ...
        if global_config.get("magi_preserve_atom_order", False):
            description_list, already_fragment = sorted_descriptions, 0
        else:
    ...
    try:
        ...
        if await knowledge_graph_inst.has_edge(src_id, tgt_id):
            ...
            if already_edge:
                ...
                if already_edge.get(
                    "description"
                ) is not None and not global_config.get(
                    "magi_projection_replace", False
                ):
        ...
        if global_config.get("magi_preserve_atom_order", False):
            sorted_edges = sorted(
                (dp for dp in edges_data if dp.get("description")),
                key=lambda item: item.get("magi_atom_order", 0),
            )
        else:
            unique_edges = {}
...
async def extract_entities(
    ...
    magi_memory_enabled = bool(global_config.get("magi_memory_enabled", False))
    # MAGI's Atom/time contract requires structured output. Compatibility mode
    # retains the caller's original JSON/text extraction preference.
    use_json_extraction = bool(
        global_config.get("entity_extraction_use_json", False) or magi_memory_enabled
    )
    ...
    async def _process_single_content(chunk_key_dp: tuple[str, TextChunkSchema]):
        ...
        episode_context_getter = global_config.get("memory_episode_context_getter")
        episode_context = (
            episode_context_getter(chunk_dp.get("full_doc_id"))
            if callable(episode_context_getter)
            else {}
        ) or {}
        chunk_context = {
            **context_base,
            "reference_at": episode_context.get(
        ...
        if magi_memory_enabled:
            entity_extraction_system_prompt = PROMPTS[
                "magi_atom_extraction_json_system_prompt"
            ].format(**chunk_context)
            entity_extraction_user_prompt = PROMPTS[
                "magi_atom_extraction_json_user_prompt"
            ].format(
                **{
                    **chunk_context,
                    "input_text": content,
            ...
            entity_continue_extraction_user_prompt = PROMPTS[
                "magi_atom_continue_extraction_json_user_prompt"
            ].format(**chunk_context)
        ...
        if use_json_extraction:
            maybe_nodes, maybe_edges = await _process_json_extraction_result(
                ...
                timestamp,
                file_path,
                magi_memory_enabled=magi_memory_enabled,
            )
        else:
        ...
        # LightRAG compatibility mode may make one extra "gleaning" request.
        # MAGI's structured prompt already requires the complete Episode ->
        # entity/relation/Atom result in one response; replaying that response
        # adds latency, creates partially populated duplicate containers, and
        # violates the write pipeline's one extraction-call contract.
        run_gleaning = entity_extract_max_gleaning > 0 and not magi_memory_enabled
        if (
            run_gleaning
        ...
        if run_gleaning:
            ...
            for i, (entity_name, glean_entities) in enumerate(
                ...
                if entity_name in maybe_nodes and magi_memory_enabled:
                    existing = {
                        (
                            item.get("description"),
                            json.dumps(
                                item.get("atom_payload"),
                                ensure_ascii=False,
                                sort_keys=True,
                            ),
                        )
                        for item in maybe_nodes[entity_name]
                    }
                    maybe_nodes[entity_name].extend(
                        item
                        for item in glean_entities
                        if (
                            item.get("description"),
                            json.dumps(
                                item.get("atom_payload"),
                ...
                elif entity_name in maybe_nodes:
                    # Compare description lengths and keep the better one
            ...
            for i, (edge_key, glean_edge_list) in enumerate(
                ...
                if edge_key in maybe_edges and magi_memory_enabled:
                    existing = {
                        (
                            item.get("description"),
                            json.dumps(
                        ...
                        for item in maybe_edges[edge_key]
                    }
                    maybe_edges[edge_key].extend(
                        item
                        for item in glean_edge_list
                        if (
                            item.get("description"),
                            json.dumps(
                                item.get("atom_payload"),
