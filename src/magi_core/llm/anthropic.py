...
async def anthropic_complete_if_cache(
    ...
    **kwargs: Any,
    ...
    if image_inputs:
        from magi_core.llm._vision_utils import normalize_image_inputs
...
# ``magi_core.llm.voyageai.voyageai_embed``. Keep the old name importable for one
...
def anthropic_embed(*args, **kwargs):
    """Deprecated alias for :func:`magi_core.llm.voyageai.voyageai_embed`.
    ...
    warnings.warn(
        "magi_core.llm.anthropic.anthropic_embed is deprecated and will be "
        "removed in a future release. Import "
        "magi_core.llm.voyageai.voyageai_embed instead.",
        DeprecationWarning,
        stacklevel=2,
    )
