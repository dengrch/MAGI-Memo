...
from magi_core.kg import STORAGES
...
def get_storage_class(storage_name: str) -> Callable[..., Any]:
    ...
    if storage_name == "NanoVectorDBStorage":
        from magi_core.kg.nano_vector_db_impl import NanoVectorDBStorage

        return NanoVectorDBStorage
    if storage_name == "NetworkXStorage":
        from magi_core.kg.networkx_impl import NetworkXStorage
    ...
    # against the top-level ``magi_core`` package, so anchor the import there
    # rather than letting it resolve against this module's own package.
    import_path = STORAGES[storage_name]
    module = importlib.import_module(import_path, package="magi_core")
