...
def direct_log(message, enable_output: bool = True, level: str = "DEBUG"):
    ...
    # Get the current logger level from the magi_core logger
    try:
        from magi_core.utils import logger

        current_level = logger.getEffectiveLevel()
    except ImportError:
        # Fallback if magi_core.utils is not available
...
# Workspace pipeline ingress registry (see magi_core.kg.pipeline_ingress).
...
def _ensure_ingress_on_current_loop(ingress: Any, final_namespace: str) -> None:
    ...
    Follows the sync-wrapper convention (see ``magi_core.py``: an object built
...
def _debug_log_failure(message: str, exc: Exception) -> None:
    ...
    """
    try:
        logging.getLogger("magi_core").debug("pipeline status log: %s: %r", message, exc)
    except Exception:
        pass
...
async def acquire_enqueue_reservation(
    ...
    * no capacity → :class:`~magi_core.exceptions.PipelineBackpressureError`
