...
``NativeParserBase.parse`` fixes the common local-parse flow once:
    ...
    → persist full_docs (magi_core) → archive source
...
from magi_core.constants import FULL_DOCS_FORMAT_LIGHTRAG
from magi_core.parser.base import BaseParser, ParseContext, ParseResult
...
class NativeParserBase(BaseParser):
    ...
    def _build_llm_submit(
        ...
        from magi_core.utils import get_llm_cache_identity, use_llm_func_with_cache
    ...
    def finalize_parse_warnings(
        ...
        *,
        ...
        :class:`~magi_core.parser.docx.parser.NativeDocxParser`). The base stays
    ...
    async def parse(self, ctx: ParseContext) -> ParseResult:
        from magi_core.parser.routing import decode_parse_engine, encode_parse_engine
        from magi_core.sidecar import write_sidecar
