...
from magi_core.constants import NATIVE_RAW_DIR_SUFFIX, PARSER_ENGINE_NATIVE
from magi_core.parser.external._common import raw_dir_for_parsed_dir
from magi_core.parser.markdown.extract import ResolvedImage, extract_markdown
from magi_core.parser.markdown.raw_cache import (
    NativeImageRawCache,
    native_md_options_signature,
)
from magi_core.parser.native_base import NativeExtractRuntime, NativeParserBase
from magi_core.utils import logger

if TYPE_CHECKING:
    from magi_core.sidecar.ir import IRDoc
...
class _MarkdownImageResolver:
    """Concrete :class:`~magi_core.parser.markdown.extract.ImageResolver`.
    ...
    def _download(self, src: str) -> tuple[bytes, str]:
        ...
        req = urllib.request.Request(src, headers={"User-Agent": "magi_core-native-md"})
...
class NativeMarkdownParser(NativeParserBase):
    ...
    def _open_textpack(self, source: Path, tmp_dir: Path) -> tuple[str, Path]:
        from magi_core.parser.external._zip import safe_extract_zip
    ...
    def build_ir(
        ...
        *,
        ...
        from magi_core.parser.markdown.ir_builder import NativeMarkdownIRBuilder
