...
(:mod:`magi_core.parser.registry`) instead of a growing ``if engine == …``
chain.
...
if TYPE_CHECKING:
    from magi_core.sidecar.ir import IRDoc  # noqa: F401
...
class ParseContext:
    ...
    async def archive_source(self, source_path: str) -> str | None:
        ...
        ``magi_core.pipeline.archive_docx_source_after_full_docs_sync`` keep
