...
from magi_core.constants import FULL_DOCS_FORMAT_RAW, PARSER_ENGINE_LEGACY
...
class LegacyParser(BaseParser):
    ...
    async def parse(self, ctx: ParseContext) -> ParseResult:
        from magi_core.parser.legacy.extractors import (
            LegacyExtractionError,
            extract_text,
        )
        from magi_core.parser.registry import suffix_capabilities
