...
shared by the unified parser debug CLI (``magi_core/parser/cli.py``),
...
def build_debug_rag(*, extract_llm_func=None):
    ...
    init (the parser CLI resolves ``magi_core.parser.debug`` before
    ``magi_core`` itself is fully bound).
