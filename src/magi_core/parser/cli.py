...
Invocation::
    ...
    python -m magi_core.parser.cli path/to/sample.docx --engine native
    python -m magi_core.parser.cli path/to/sample.pdf  --engine mineru
    python -m magi_core.parser.cli path/to/sample.pdf  --engine docling --force-reparse
...
def _normalize_direct_script_sys_path() -> None:
    ...
    # Direct execution adds magi_core/parser to sys.path, which makes the
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    from magi_core.parser.registry import supported_parser_engines
...
async def _run(args: argparse.Namespace) -> int:
    ...
    from magi_core.constants import FULL_DOCS_FORMAT_PENDING_PARSE
    from magi_core.parser.base import ParseContext
    from magi_core.parser.external._base import ExternalParserBase
    from magi_core.parser.registry import get_parser, suffix_capabilities
    from magi_core.parser.debug import build_debug_rag
    from magi_core.parser.docx.parse_document import DocxContentError
    from magi_core.utils import compute_mdhash_id
    import magi_core.pipeline as pipeline_mod
    import magi_core.utils_pipeline as utils_pipeline_mod
    ...
    with ExitStack() as stack:
        ...
        # ctx.archive_source -> magi_core.pipeline.archive_docx_source_after_...
        stack.enter_context(
            mock.patch.object(
                pipeline_mod,
...
def main(argv: list[str] | None = None) -> int:
    ...
    # so a `magi_core.parsers` entry-point engine is selectable here exactly
