...
- :class:`ReuseParser` handles ``magi_core`` rows — a document already parsed
...
from magi_core.constants import FULL_DOCS_FORMAT_LIGHTRAG, FULL_DOCS_FORMAT_RAW
from magi_core.parser.base import BaseParser, ParseContext, ParseResult


class ReuseParser(BaseParser):
    """Reuse an already-parsed (``magi_core``-format) document's sidecar."""
