...
from magi_core.namespace import NameSpace
from magi_core.ingestion import KnowledgeCommitContext, KnowledgeIngestionAdapter
from magi_core.chunker import chunking_by_token_size
...
from magi_core.utils_pipeline import (
...
from magi_core.exceptions import IndexFlushError
from magi_core.utils import (
    Tokenizer,
    TiktokenTokenizer,
    EmbeddingFunc,
...
from magi_core.types import KnowledgeGraph
from dotenv import load_dotenv
from magi_core.pipeline import _PipelineMixin
from magi_core.kg.factory import get_storage_class
from magi_core.addon_params import (
    ObservableAddonParams,
    normalize_addon_params,
)
from magi_core.llm_roles import (
    ROLE_NAMES,
    ROLES,
    RoleLLMConfig,
    RoleSpec,  # noqa: F401  # re-exported via magi_core/__init__.py
    _optional_env_int,
    _RoleLLMMixin,
    _RoleLLMState,
)
from magi_core.storage_migrations import _StorageMigrationMixin
...
def _run_sync(
    ...
    * **A different — but still alive — loop than the storages bound to**
      ...
      ``asyncio.Lock`` objects in ``magi_core.kg.shared_storage`` are bound to
...
@final
@dataclass
class MagiCore(_RoleLLMMixin, _StorageMigrationMixin, _PipelineMixin):
    """MAGI Core memory engine evolved from the tested LightRAG core."""
    ...
    :func:`magi_core.parser.routing.default_chunker_config`.  Per-doc
    ...
    **When this function is actually invoked.** The chunker dispatch in
    ...
    document's ``process_options``:
      ...
      - If ``process_options`` explicitly contains a chunking selector
        ...
        :mod:`magi_core.chunker` (``chunking_by_fixed_token`` for ``F``,
    ...
    :attr:`magi_core.parser.routing.ProcessOptions.chunking_explicit`.
    ...
    **Signature** — preserved unchanged from earlier LightRAG releases
    ...
    Defaults to :func:`magi_core.chunker.chunking_by_token_size`.
    """
    ...
    :class:`~magi_core.exceptions.PipelineBackpressureError` (HTTP 429) when
    ...
    """If True, magi_core will automatically calls initialize_storages and finalize_storages at the appropriate times."""
    ...
    def _apply_chunk_size_overlay(self) -> None:
        ...
        2. Strategy-specific env (``CHUNK_F_SIZE`` / ``CHUNK_R_SIZE`` /
           ...
           :func:`magi_core.parser.routing.default_chunker_config` when it
    ...
    def _build_global_config(self) -> dict[str, Any]:
        ...
        adapter = getattr(self, "_knowledge_ingestion_adapter", None)
        global_config["magi_memory_enabled"] = adapter is not None
        global_config["memory_episode_context_getter"] = getattr(
            adapter, "get_extraction_context", None
        )
    ...
    def __post_init__(self, addon_params: dict[str, Any] | None):
        ...
        # Runtime-only integration point. It is deliberately not a dataclass
        # field, so _build_global_config()/asdict() never copies an adapter that
        # may own database handles, locks, or other live resources.
        self._knowledge_ingestion_adapter: KnowledgeIngestionAdapter | None = None
    ...
    async def initialize_storages(self):
        ...
        if self._storages_status == StoragesStatus.CREATED:
            ...
            from magi_core.kg.shared_storage import initialize_pipeline_status
    ...
    async def ainsert(
        ...
        from magi_core.parser.routing import resolve_chunk_options
        ...
        try:
            ...
            async with get_storage_keyed_lock(
                ...
                try:
                    ...
                    await self._prepare_knowledge_episode(
                        doc_id=doc_key,
                        content=full_text,
                        file_path=file_path,
                        reference_at=(status_row or {}).get("created_at"),
                    )
                    ...
                    if chunk_results:
                        await self._commit_extracted_knowledge(
                    ...
                    await self._complete_knowledge_episode(doc_key)
                    ...
                    # then re-raise. The configured knowledge commit path must
                    # drain any sibling writes before propagating a failure.
                    op_failed = True
                    try:
                        failed_journal = {
                            **journal,
                        ...
                        await self._fail_knowledge_episode(
                            doc_key, error=str(op_exc)
                        )
    ...
    def register_knowledge_ingestion_adapter(
        self, adapter: KnowledgeIngestionAdapter | None
    ) -> None:
        """Register an optional post-extraction knowledge commit adapter.

        Passing ``None`` restores the original LightRAG merge behavior. The
        adapter is runtime state and is intentionally excluded from serialized
        configuration.
        """

        self._knowledge_ingestion_adapter = adapter

    async def _prepare_knowledge_episode(
        self,
        *,
        doc_id: str,
        content: str,
        ...
        adapter = self._knowledge_ingestion_adapter
        prepare = getattr(adapter, "prepare_episode", None)
        if callable(prepare):
            await prepare(
                doc_id=doc_id,
                content=content,
    ...
    async def _complete_knowledge_episode(
        self, doc_id: str, *, track_id: str | None = None
    ) -> None:
        adapter = self._knowledge_ingestion_adapter
        complete = getattr(adapter, "complete_episode", None)
        if callable(complete):
            await complete(doc_id, track_id=track_id)

    async def _fail_knowledge_episode(
        self, doc_id: str, *, error: str
    ) -> None:
        adapter = self._knowledge_ingestion_adapter
        fail = getattr(adapter, "fail_episode", None)
        if callable(fail):
            await fail(doc_id, error=error)

    async def _delete_knowledge_episode(self, doc_id: str) -> str | None:
        adapter = self._knowledge_ingestion_adapter
        delete = getattr(adapter, "delete_episode", None)
        if callable(delete):
            return await delete(doc_id, rag=self)
        return None
    ...
    async def _commit_extracted_knowledge(
        ...
        *,
        ...
        """Commit one extraction result through the configured integration."""
    ...
    async def ainsert_custom_kg(
        ...
        .. warning:: (issue #3400 Phase 5 — direct-writer audit)
           ...
           offline ``magi_core.tools.kg_integrity_repair`` audit can
        ...
        **Concurrency Control Design:**
        ...
        try:
            ...
            if not chunk_ids:
                ...
                try:
                    deletion_stage = "delete_magi_memory"
                    await self._delete_knowledge_episode(doc_id)
            ...
            # Remove Episode evidence only after graph deletion/rebuild has
            # succeeded. The SQLite operation writes a restorable backup and
            # refreshes Atom IDs/descriptions for every surviving owner.
            try:
                deletion_stage = "delete_magi_memory"
                await self._delete_knowledge_episode(doc_id)
            except Exception as e:
                logger.error(f"Failed to delete MAGI memory for {doc_id}: {e}")
                raise Exception(f"Failed to delete MAGI memory: {e}") from e
    ...
    async def _raise_if_recovery_required(self) -> None:
        ...
        """
        from magi_core.exceptions import PipelineNotInitializedError
    ...
    async def adelete_by_entity(self, entity_name: str) -> DeletionResult:
        ...
        from magi_core.utils_graph import adelete_by_entity
    ...
    async def adelete_by_relation(
        ...
        from magi_core.utils_graph import adelete_by_relation
    ...
    async def get_relation_info(
        ...
        """
        from magi_core.utils_graph import get_relation_info
    ...
    async def aedit_entity(
        ...
        from magi_core.utils_graph import aedit_entity
    ...
    async def aedit_relation(
        ...
        from magi_core.utils_graph import aedit_relation
    ...
    async def acreate_entity(
        ...
        from magi_core.utils_graph import acreate_entity
    ...
    async def acreate_relation(
        ...
        from magi_core.utils_graph import acreate_relation
    ...
    async def amerge_entities(
        ...
        from magi_core.utils_graph import amerge_entities
    ...
    async def aexport_data(
        ...
        """
        from magi_core.utils import aexport_data as utils_aexport_data
...
# passed through MagiCore(addon_params=...). InitVars are not stored as
...
MagiCore.addon_params = property(  # type: ignore[attr-defined]
    MagiCore._get_addon_params,
    MagiCore._set_runtime_addon_params,
)

# Temporary source-compatibility alias for the retained API server, existing
# provider examples, and downstream users. New MAGI code should import
# ``MagiCore`` directly.
LightRAG = MagiCore
