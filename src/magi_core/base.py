...
class BaseVectorStorage(StorageNameSpace, ABC):
    ...
    async def upsert(self, data: dict[str, dict[str, Any]]) -> None:
        ...
        Multi-worker note:
            ...
            magi_core-gunicorn) other workers will not observe these writes
...
class BaseKVStorage(StorageNameSpace, ABC):
    ...
    async def get_by_id_strict(self, id: str) -> dict[str, Any] | None:
        ...
        * Any transport/server error, an index/collection that is not ready,
        ...
        :class:`~magi_core.exceptions.StorageCapabilityError`; callers must gate
    ...
    async def upsert(self, data: dict[str, dict[str, Any]]) -> None:
        ...
        Multi-worker note:
            ...
            deployment (e.g. magi_core-gunicorn) other workers will not
...
class DocStatusStorage(BaseKVStorage, ABC):
    ...
    async def count_docs_by_statuses(
        ...
        :class:`~magi_core.exceptions.StorageCapabilityError`; deployments that
    ...
    async def update_doc_status_fields(
        ...
        * Unknown ``doc_id`` raises
          :class:`~magi_core.exceptions.StorageRecordNotFoundError` unless
    ...
    async def list_source_conflicts_page(
        ...
        *,
        ...
        :class:`~magi_core.exceptions.StorageCapabilityError`; only backends
    ...
    async def repair_source_conflict(
        ...
        * On commit, the current candidate set is re-read strictly and a
          ...
          :class:`~magi_core.exceptions.SourceConflictRepairCASError` rather
        ...
        **What this method does NOT serialize.** The storage layer holds no
        ...
        ``magi_core.tools.source_conflict_repair.source_conflict_repair_lock``
        ...
        :class:`~magi_core.exceptions.StorageCapabilityError`.
        """
        raise StorageCapabilityError(
