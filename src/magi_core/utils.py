...
logger = logging.getLogger("magi_core")
...
if TYPE_CHECKING:
    from magi_core.base import BaseKVStorage, BaseVectorStorage, QueryParam
...
class LightragPathFilter(logging.Filter):
    """Filter for magi_core logger to filter out frequent path access logs"""
...
def setup_logger(
    ...
    Args:
        ...
        log_file_path: Path to the log file. If None and file logging is enabled, defaults to magi_core.log in LOG_DIR or cwd
...
class VectorStorageConsistencyError(Exception):
    ...
    server and run the offline rebuild tool (``magi_core-rebuild-vdb``) to restore
    consistency.
    """
...
def priority_limit_async_func_call(
    ...
    def final_decro(func):
        ...
        publish_stats = False
        shared = None  # lazily imported magi_core.kg.shared_storage module
        ...
        def _resolve_mode() -> bool:
            ...
            if concurrency_group is None:
                ...
                return True
            if shared is None:
                from magi_core.kg import shared_storage as shared_module
...
def write_json(json_obj, file_name):
    ...
    """
    from magi_core.file_atomic import atomic_write
...
def _normalized_child_offsets(
    ...
    projection :mod:`magi_core.sidecar.backfill` uses.
    """
...
class TruncatedResponse(str):
    ...
    in ``use_llm_func_with_cache`` and ``magi_core.operate``.
    """
