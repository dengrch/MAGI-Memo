...
from magi_core.utils import setup_logger, get_env_value
...
worker_class = "magi_core.api.gunicorn_worker.LightRAGUvicornWorker"

# Other Gunicorn configurations

# Logging configuration
errorlog = os.getenv("ERROR_LOG", log_file_path)  # Default write to magi_core.log
accesslog = os.getenv("ACCESS_LOG", log_file_path)  # Default write to magi_core.log
...
def on_starting(server):
    ...
    # Log the location of the MAGI Memo log file
    print(f"MAGI Memo log file: {log_file_path}\n")
...
def post_fork(server, worker):
    ...
    setup_logger("magi_core", log_level, add_filter=True, log_file_path=log_file_path)

    # Set up magi_core submodule loggers
    for name in logging.root.manager.loggerDict:
        if name.startswith("magi_core."):
