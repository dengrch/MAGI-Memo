...
to :data:`magi_core.constants.DEFAULT_R_SEPARATORS`); only when a single
...
:func:`magi_core.chunk_schema.strip_internal_multimodal_markup_for_extraction`
...
``magi_core.operate._process_single_content``) and ensures the
...
from magi_core.chunk_schema import strip_internal_multimodal_markup_for_extraction
from magi_core.constants import DEFAULT_R_SEPARATORS
...
from magi_core.utils import Tokenizer
...
def load_chunk_separators() -> list[str]:
    ...
    :data:`magi_core.constants.DEFAULT_R_SEPARATORS` on missing / invalid
...
def trim_content_to_budget(
    ...
    *,
    ...
    :data:`magi_core.constants.DEFAULT_MAX_EXTRACT_INPUT_TOKENS`.  Only ``content``
