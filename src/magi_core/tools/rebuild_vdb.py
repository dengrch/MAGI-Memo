...
Usage:
    magi_core-rebuild-vdb
    # or
    python -m magi_core.tools.rebuild_vdb
...
from magi_core.kg import STORAGE_ENV_REQUIREMENTS
...
async def rebuild_entities_vdb(
    ...
    *,
    ...
    """
    from magi_core.operate import _truncate_vdb_content
...
async def enumerate_kv_keys(kv) -> List[str]:
    ...
    scans (same patterns as magi_core/tools/clean_llm_query_cache.py).
    ...
    if storage_name == "PGKVStorage":
        from magi_core.kg.postgres_impl import namespace_to_table_name
...
class RebuildTool:
    ...
    def resolve_storage_names(self) -> Dict[str, str]:
        return {
            "graph": os.getenv("LIGHTRAG_GRAPH_STORAGE", "Neo4JStorage"),
    ...
    def build_embedding_func(self) -> EmbeddingFunc | None:
        ...
        """
        try:
            from magi_core.api.config import global_args
            from magi_core.api.lightrag_server import (
                create_embedding_function_from_args,
            )
        except ImportError as e:
            print(f"\n⚠️  Could not import the LightRAG API package: {e}")
            print('   Rebuild requires the api extra: pip install "magi_core-hku[api]"')
    ...
    def build_global_config(self) -> Dict[str, Any]:
        ...
        if max_token_size:
            try:
                from magi_core.utils import TiktokenTokenizer
        ...
        if not self.embedding_available:
            ...
            async def _no_embedding(*_args, **_kwargs):
                raise RuntimeError(
                    ...
                    'Install the api extra: pip install "magi_core-hku[api]"'
                )
            ...
            for storage in self.all_storages():
                if storage is not None:
                    try:
                        ...
                        pass
            try:
                from magi_core.kg.shared_storage import finalize_share_data
...
def main():
    ...
    setup_logger("magi_core", level="INFO")
