...
Two contracts coexist intentionally:
  ...
  - **Legacy contract** — :func:`chunking_by_token_size` keeps its
    ...
    so externally-supplied :attr:`magi_core.LightRAG.chunking_func`
  ...
  - **File-chunker contract** — for documents whose ``process_options``
    ...
    Currently shipped file chunkers:
      ...
      - :func:`chunking_by_semantic_vector` — the ``"V"`` strategy.
        ...
        :class:`~magi_core.utils.EmbeddingFunc`.
...
from magi_core.chunker.paragraph_semantic import chunking_by_paragraph_semantic
from magi_core.chunker.recursive_character import (
    chunking_by_recursive_character,
)
from magi_core.chunker.semantic_vector import chunking_by_semantic_vector
from magi_core.chunker.token_size import (
