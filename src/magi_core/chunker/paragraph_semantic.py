...
:func:`magi_core.chunker.chunking_by_token_size`.
...
from magi_core.table_markup import (
...
from magi_core.utils import Tokenizer, logger


# ---------------------------------------------------------------------------
# Threshold ratios — derived from the audit-mode constants in
# magi_core/parser/docx/parse_document.py so the trade-off curves
...
# imported from :mod:`magi_core.table_markup` so the surrounding-context
...
# Mirrors ``_MD_HEADING_RE`` in ``magi_core/parser/_markdown.py`` (the same
...
def _tables_sidecar_path(blocks_path: str) -> str | None:
    ...
    Mirrors :func:`magi_core.utils_pipeline.sidecar_modality_path` naming
...
def _character_split_text(
    ...
    *,
    ...
    """
    from magi_core.chunker.recursive_character import (
...
def _split_long_block(
    ...
    *,
    ...
    Mirrors :func:`magi_core.parser.docx.parse_document.split_long_block`,
...
def _is_heading_only(block: dict[str, Any]) -> bool:
    ...
    accepted ambiguity documented in ``magi_core/parser/_markdown.py`` and is
...
def _merge_small_blocks(
    ...
    *,
    ...
    :func:`magi_core.parser.docx.parse_document.merge_small_blocks`,
