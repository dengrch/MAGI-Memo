...
Caveats:
  - SemanticChunker does NOT enforce a maximum chunk size; the caller's
    ...
    :func:`magi_core.utils.enforce_chunk_token_limit_before_embedding`.
  - When ``embedding_func`` is ``None`` we log a warning and fall back to
    :func:`magi_core.chunker.chunking_by_recursive_character` — V's only
...
from magi_core.utils import EmbeddingFunc, Tokenizer, logger
