...
Why this lives at the package root rather than under ``magi_core/kg/``:
``magi_core.utils.write_json`` needs ``atomic_write`` to gain crash safety,
and several ``magi_core/kg/*`` modules need both. Hosting the helpers under
``magi_core/kg/`` would create a ``utils -> kg -> utils`` import cycle.
