...
``magi_core.py`` stays focused on storage management, querying, and editing.
The mixin is wired into :class:`magi_core.LightRAG` via multiple inheritance
...
from magi_core.kg.pipeline_ingress import PipelineIngressMessage
from magi_core.parser.base import ParseContext
from magi_core.parser.llm_bridge import LLMBridgePipelineCancelled
from magi_core.parser.registry import (
    get_parser,
...
from magi_core.utils_pipeline import (
    ...
    # directly): the parser layer resolves these as ``magi_core.pipeline.<name>``
...
def _vlm_image_budget_limits() -> tuple[int, int]:
    ...
    from magi_core.constants import DEFAULT_MM_IMAGE_MIN_PIXEL
...
class _PipelineMixin:
    ...
    async def _reserve_ingress_slot(
        ...
        * **the reservation is what makes the write side mutually exclusive with
        ...
        """
        from magi_core.kg.shared_storage import acquire_enqueue_reservation
    ...
    async def _release_admission_reservation(self, token: str) -> None:
        ...
        from magi_core.kg.shared_storage import release_token_set_reservation
    ...
    async def apipeline_enqueue_documents(
        ...
        Args:
            ...
            chunk_options: per-document chunker parameter snapshot.
                ...
                :func:`magi_core.parser.routing.resolve_chunk_options`
        ...
        if docs_format not in (FULL_DOCS_FORMAT_RAW, FULL_DOCS_FORMAT_PENDING_PARSE):
            raise ValueError(
                ...
                "The 'magi_core' enqueue format was removed; already-parsed "
        ...
        def _process_options_at(index: int) -> str:
            if process_options is None:
                return ""
            from magi_core.parser.routing import sanitize_process_options
        ...
        def _chunk_options_at(index: int) -> dict[str, Any]:
            ...
            :func:`magi_core.parser.routing.resolve_chunk_options` itself
        ...
        # 1. Validate ids and build contents (when magi_core: no content dedup, content may be empty)
        if ids is not None:
    ...
    async def _feeder_next_batch(self, ingress) -> list[PipelineIngressMessage]:
        ...
        Two ingress flavors (see :mod:`magi_core.kg.pipeline_ingress`):
    ...
    async def _parse_worker(
        ...
        while True:
            ...
            try:
                ...
                # reuse/passthrough (raw/magi_core/unknown_source) are skipped.
                if (
                ...
                # has already persisted the parsed body to full_docs (magi_core /
    ...
    async def _analyze_worker(self, ctx: _BatchRunContext) -> None:
        ...
        body (pending_parse → magi_core / raw documents start with empty
    ...
    async def process_single_document(
        ...
        *,
        ...
        """
        from magi_core.parser.routing import parse_process_options
        ...
        async with ctx.semaphore:
            try:
                ...
                # strip the magi_core marker according to the stored parse_format
                ...
                await self._prepare_knowledge_episode(
                    ...
                    content=content,
                    file_path=file_path,
                    reference_at=getattr(status_doc, "created_at", None),
                )
                ...
                # parse_format=raw, which silently mislabelled magi_core docs;
                ...
                if blocks_path and sidecar_backfill_eligible:
                    from magi_core.sidecar import backfill_chunk_sidecars
            ...
            if file_extraction_stage_ok:
                try:
                    ...
                    await self._complete_knowledge_episode(
                        doc_id,
    ...
    async def _purge_stale_extraction_if_resuming(
        ...
        *,
        ...
        from magi_core.parser.routing import normalize_parser_engine
    ...
    async def _finalize_doc_failure(
        ...
        *,
        ...
        await self._fail_knowledge_episode(doc_id, error=doc_error_msg)
    ...
    async def _persist_parsed_full_docs(
        ...
        # for magi_core-format) so cross-filename dedup fires regardless of
        # whether the same body was ingested as raw text or via a sidecar.
        # ``strip_lightrag_doc_prefix`` is a no-op for non-magi_core formats.
            ...
            if source_name:
                for candidate in matches:
                    if candidate.name == source_name:
                        return str(candidate)
            if parser_engine:
                from magi_core.parser.routing import filename_parser_directives
    ...
    async def analyze_multimodal(
        ...
        *,
        ...
        if enabled_modalities and tokenizer is not None:
            try:
                from magi_core.multimodal_context import (
        ...
        try:
            ...
            from magi_core.llm._vision_utils import (
                image_audit_metadata,
                image_cache_metadata,
                normalize_image_inputs,
                read_image_dimensions,
            )
            from magi_core.prompt_multimodal import (
                IMAGE_TYPE_ENUM,
                IMAGE_TYPE_FALLBACK,
                MULTIMODAL_PROMPTS,
            ...
            # in magi_core.utils).  When the flag is off we must NOT save the
    ...
    def _build_mm_chunks_from_sidecars(
        ...
        def _render(
            ...
            # ``magi_core.operate._parse_mm_display_name`` (regex
