...
These helpers are shared by the LightRAG pipeline mixin (magi_core/pipeline.py)
...
from magi_core.exceptions import StorageCapabilityError
from magi_core.parser.routing import canonicalize_parser_hinted_basename
...
def resolve_doc_status_parse_engine(
    ...
    structured ``magi_core`` format and ``legacy`` for everything else
...
async def source_candidate_set_lock(workspace: str, canonical_source_key: str):
    ...
    """
    from magi_core.constants import SOURCE_CONFLICT_LOCK_NAMESPACE
    from magi_core.kg.shared_storage import get_storage_keyed_lock
...
def make_lightrag_doc_content(merged_text: str) -> str:
    """Build the ``full_docs.content`` value for ``format=magi_core`` records.

    The result has shape ``"{{LRdoc}}<merged_text>"`` — the marker prefix
    distinguishes magi_core-format full_docs from raw-format ones, and the
    body is the complete merged text from the ``.blocks.jsonl`` content
    lines so F-chunking can run identically on raw and magi_core inputs
...
def strip_lightrag_doc_prefix(content: str | None, parse_format: str | None) -> str:
    ...
    indicates the record is in magi_core format.  Any other ``parse_format``
...
# :func:`magi_core.operate.extract_entities._process_single_content` so each
