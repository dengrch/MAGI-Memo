...
###############################################################################
# MAGI Episode -> Atom extraction
# Enabled only when a strict MAGI knowledge adapter is registered.
###############################################################################

PROMPTS["magi_atom_extraction_json_system_prompt"] = """---Role---
You are the memory extraction component of MAGI Core. Convert one Episode chunk
into canonical entity candidates, relation candidates, and atomic memories.

---Reference Time---
The Episode reference time is `{reference_at}`. Use it as the temporal anchor
for relative expressions such as "today", "next week", or "three years ago".

---Instructions---
1. Extract meaningful entities and direct binary relationships only from the
   fenced input text. Use the entity type guidance below. Within this Episode,
   emit one entity container per real entity and collect spelling,
   abbreviation, title, and translated-name variants in its `aliases` array.
2. Do not write entity or relationship summaries. Instead, emit an `atoms`
   array for every entity and relationship. Each Atom must contain exactly one
   independently judgeable, self-contained fact.
3. Atom `content` must explicitly name its subject and relevant object. Do not
   use context-dependent pronouns. Preserve negation, quantity, modality,
   conditions, and direction. Include temporal wording in `content` when it is
   semantically important.
4. Normalize temporal fields to ISO-8601 with an explicit UTC offset:
   - `valid_at`: when the fact first becomes true. Infer it from the text and
     reference time. Use the Episode reference time when the fact is asserted
     as current but no more precise time is available. Use null only for truly
     timeless definitional facts.
   - `invalid_at`: when the fact stops being true. Set it only when the text
     explicitly supplies an end, replacement, correction, or expiry. Otherwise
     use null. Never use extraction time as an automatic invalidation time.
   - `temporal_text`: the exact short time expression that justified the
     inference, or null.
   - `temporal_precision`: one of `exact`, `day`, `month`, `year`, `relative`,
     `inferred_current`, or null.
5. Entity Atom fields: `content`, `valid_at`, `invalid_at`, `temporal_text`,
   `temporal_precision`, `confidence`, and `importance`.
6. Relation Atom fields add `predicate`. The predicate is a short normalized
   relation label; the content still carries the complete meaning.
7. Confidence and importance are numbers from 0 to 1. They are separate:
   confidence measures extraction certainty; importance measures memory value.
8. Do not deduplicate against historical memory. Only remove exact repetitions
   within this response. Historical duplicate/conflict judgment happens later.
9. Treat graph relationships as undirected containers for compatibility, but
   preserve the statement's semantic direction inside `predicate` and Atom
   `content`.
10. Entity Atoms contain intrinsic or unary facts. Facts whose meaning depends
    on two entities belong only to the relationship container; do not repeat
    them as entity Atoms. Conservative, useful entity inferences that follow
    directly from the Episode are encouraged, but must be explicit in
    `content` and use appropriately lower `confidence`.
11. `invalid_at` describes the end of the emitted fact's own real-world valid
    interval. A request to forget, correct, or update previous memory does not
    mutate historical Atoms during extraction; leave that to a later merge
    policy.
12. Return one JSON object with `entities` and `relationships` arrays only.
    Do not include `description` fields and do not wrap the JSON in markdown.
13. Output at most {max_total_records} entity/relation containers and at most
    {max_entity_records} entities. Prefer facts central to the Episode.
14. Write Atom content in {language}. Preserve proper nouns when translation
...
{{
  "entities": [
    {{
      ...
      "aliases": ["<Episode-local alias>"],
      "type": "<entity type>",
      "atoms": [
        {{
          "content": "<one atomic fact>",
          "valid_at": "<ISO-8601 or null>",
          "invalid_at": "<ISO-8601 or null>",
          "temporal_text": "<source expression or null>",
          "temporal_precision": "<precision or null>",
          "confidence": 0.0,
          "importance": 0.0
        }}
      ]
    ...
    {{
      ...
      "atoms": [
        {{
          ...
          "content": "<one directed atomic relation fact>",
          "valid_at": "<ISO-8601 or null>",
          "invalid_at": "<ISO-8601 or null>",
          "temporal_text": "<source expression or null>",
          "temporal_precision": "<precision or null>",
...
PROMPTS["magi_atom_extraction_json_user_prompt"] = """---Task---
Extract entity candidates, relation candidates, and their atomic memories from
the Episode chunk below. Follow the Atom and temporal contract exactly.
...
PROMPTS["magi_atom_continue_extraction_json_user_prompt"] = """---Task---
Return one JSON object containing only high-value entity/relation containers or
Atoms that were missed or malformed in the prior response. Do not repeat valid
prior output. Use the same MAGI Atom schema. If nothing is missing, return
{{"entities": [], "relationships": []}}.
"""

PROMPTS["magi_entity_resolution_prompt"] = """---Role---
Resolve every entity extracted from one Episode against an already-disambiguated
entity registry in one batch. Existing candidates must never be merged with
each other.

---Episode Entity Requests---
{new_entities}
...
Return JSON only:
{{"resolutions": [
  {{"key": "<request key>", "entity_id": "<one candidate id or null>",
    "confidence": 0.0, "canonical_name": "<new name or null>",
    "reason": "<brief reason>"}}
]}}

Return exactly one resolution per request key. Choose an id only when both
mentions refer to the same real entity. Similar
topics, namesakes, organizations with related names, versions, and parent/child
entities are not automatically identical. Return null when evidence is weak.
When returning a null entity_id for a namesake, provide a concise unambiguous
canonical_name grounded in the Episode; do not copy an existing candidate name.
"""

PROMPTS["magi_atom_resolution_prompt"] = """---Role---
Resolve every new Atom from one Episode against its allowed stored candidates in
one batch. Each new Atom and candidate belongs to an already-disambiguated entity
or relation owner. Never match across owners and never select an Atom outside the
request's candidate_atom_ids.

---Temporal contract---
1. valid_at/invalid_at are valid-time fields: when a fact becomes and stops being
   true in the real world.
2. created_at/expired_at are system-time fields: when MAGI stores an Atom and
   when that stored revision stops participating in the current projection.
3. A REFINEMENT makes a less precise stored Atom obsolete in the system but does
   not make its real-world statement false. Do not assign target_invalid_at.
4. A TEMPORAL_SUCCESSOR represents a later state. target_invalid_at should be the
   real-world transition time, normally the new Atom's valid_at. MAGI will also
   expire the old stored revision at commit time.
5. A CONTRADICTION records incompatible claims over an overlapping interval.
   Preserve both by default. Set supersedes_target=true only when the Episode
   clearly corrects/retracts the old claim or is demonstrably more authoritative;
   provide target_invalid_at only when a real-world end time is justified.
6. expired_at is never inferred by you; MAGI applies it deterministically from
   the selected operation. The new Atom's own invalid_at must not be copied to a
   different Atom unless the text explicitly establishes that target's end.

---Decision contract---
- DUPLICATE: same factual assertion; reuse the stored Atom and attach evidence.
- REFINEMENT: the new Atom materially adds precision without denying the core
  assertion; insert it and link it to the less precise stored Atom.
- TEMPORAL_SUCCESSOR: the same property/relation changes at a later time; insert
  the new Atom and end the previous state.
- CONTRADICTION: both claims cannot be true over the same interval; insert and
  link both claims, normally without deleting or expiring either.
- INDEPENDENT: distinct compatible information; insert normally.

---New Atom requests---
{new_atom_requests}

---Deduplicated candidate Atom table---
{candidate_atoms}

---Recent Episode context---
{recent_episodes}

Return exactly one decision for every new Atom that appears above. Return JSON
only:
{{"decisions": [
  {{"new_atom_id": "<new id>",
    "decision": "DUPLICATE|REFINEMENT|TEMPORAL_SUCCESSOR|CONTRADICTION|INDEPENDENT",
    "matched_atom_id": "<allowed stored candidate id or null>",
    "target_invalid_at": "<ISO-8601 or null>",
    "supersedes_target": false,
    "confidence": 0.0,
    "reason": "<brief reason>"}}
]}}
"""
